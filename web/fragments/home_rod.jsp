<%-- 
    Document   : home_rod
    Created on : Jun 20, 2020, 4:25:24 PM
    Author     : steve
--%>
                    ...
                    <div class="container">
                        <div class="content">
                            ...
                            <div style="width: 300px; padding: 5px 10px;text-align: left">
                                <h3>${product.name}</h3>
                                <p>Rod Type: <strong>${product.productDetailRod.rodType}</strong></p>
                                <p>Rod Length (feet): <strong>${product.productDetailRod.rodLength}</strong></p>                                
                                <c:if test="${not empty product.productDetailRod.rodAction}">
                                    <p>Rod Action: <strong>${product.productDetailRod.rodAction}</strong></p>
                                </c:if>
                                <c:if test="${not empty product.productDetailRod.rodPower}">
                                    <p>Rod Power: <strong>${product.productDetailRod.rodPower}</strong></p>
                                </c:if>
                                <p>Line Rating (kg): <strong>
                                        <c:if test="${product.productDetailRod.lineRatingMin !=0.0}">
                                            ${product.productDetailRod.lineRatingMin} - ${product.productDetailRod.lineRatingMax}
                                        </c:if>
                                        <c:if test="${product.productDetailRod.lineRatingMin ==0.0}">
                                            ${product.productDetailRod.lineRatingMax}
                                        </c:if>
                                    </strong></p>
                            </div>
                        ...
                        <div class="middle">
                            <input class="text" type="submit" value="Matching Reel" />                           
                        </div>
                    </div>                 
                </div>
            </c:forEach>
